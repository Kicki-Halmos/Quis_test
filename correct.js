class Correct {
    ...
    addToCheckedList(checkbox) {
     ...
     keepCheckedCheckboxes() {
        ...
        let current_question_checked = [];
        let array_to_checkedArray = [];
        ...
        div.addEventListener("click", function(event){
            ...
            if (event.target.tagName == "INPUT"){
                if(event.target.checked ){
                    current_question_checked.push(event.target.id);
                }
                else{
                    current_question_checked.splice(0,1,);
                }
            }
            //console.log(current_question_checked);
            array_to_checkedArray.splice(0,1,current_question_checked);

            
        });
        //console.log(array_to_checkedArray);
        this.addToCheckedList(array_to_checkedArray);
